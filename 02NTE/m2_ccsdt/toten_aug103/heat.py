...
def generate_sob(mol_list):
    bond_types=['N-O','N:O','N-N','N=O','N=N','N:N','N#N','C-N','C-C','C-H','C:N','C:C','C-O','C=O','C=N','C=C','H-O','H-N']
...
# Estimated data
data, mol_mass, num_mols, kojin_id = read_data('../list/Hhfnzpe_wb97mv.csv')
mol_list= read_id(kojin_id)

target_cep= np.array(data['SP energy'].values)
...
# benchmark data
data2, mol_mass2, num_mols2, kojin_id = read_data('../list/Hhfnzpe_pwpb95.csv')
#enthalpy = data2['DFT enthalpy'].values
enthalpy = data2['SP energy'].values
y = enthalpy
